...
namespace XTP {
	namespace API {
		...
		public class XtpTraderSpi :public TraderSpi {
		public:
			//MyQuoteSpi() {}
...
#pragma region 实现TraderSpi回调
			...
			void OnError(XTPRI *error_info) {
				traderevent_managed->OnError(Native::RspInfoConverter(error_info));
			}
			void OnDisconnected(UInt64 session_id, int reason) {
			...
			void OnOrderEvent(XTPOrderInfo *order_info, XTPRI *error_info,uint64_t session_id) {
				traderevent_managed->OnOrder(Native::RspInfoConverter(error_info), GetOrderInfoStruct(order_info));
			};
			///成交通知
			...
			void OnCancelOrderError(XTPOrderCancelInfo *cancel_info, XTPRI *error_info, uint64_t session_id) {
				...
				traderevent_managed->OnOrderCancel(Native::RspInfoConverter(error_info), cancel);
			};
			...
			void OnQueryOrder(XTPQueryOrderRsp *order_info, XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) {
				traderevent_managed->OnQueryOrder(Native::RspInfoConverter(error_info), GetOrderInfoStruct(order_info), request_id, is_last);
			};
			...
			void OnQueryTrade(XTPQueryTradeRsp *trade_info, XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) {
				traderevent_managed->OnQueryTrade(Native::RspInfoConverter(error_info), GetTradeReportStruct(trade_info), request_id, is_last);
			};
			...
			void OnQueryPosition(XTPQueryStkPositionRsp *position, XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) {
				...
				traderevent_managed->OnQueryPosition(Native::RspInfoConverter(error_info), posi, request_id, is_last);
			};
			...
			void OnQueryAsset(XTPQueryAssetRsp *asset, XTPRI *error_info, int request_id, bool is_last, uint64_t session_id) {
				...
				traderevent_managed->OnQueryAsset(Native::RspInfoConverter(error_info), query_asset, request_id, is_last);
			};
...
#pragma endregion
		...
		///曝露给C#的引用类 
		public ref class XtpTrader
		{
		private:
			XTP::API::TraderApi * pUserApi;
			XTP::API::TraderSpi* pUserSpi;
		public:
			XtpTrader(int client_id, String^ account_key, String ^ log_path, TE_RESUME_TYPE resume_type)
			{
				...
				pUserApi->SetSoftwareVersion("1.1.16.9");
				...
				pUserSpi = new XTP::API::XtpTraderSpi();
				Marshal::FreeHGlobal((IntPtr)log_path_char);
				//pUserApi->RegisterSpi(&spi); // 注册事件类
				traderevent_managed->OnDisconnectedEvent += gcnew XTP::API::TraderEventSource::OnDisconnectedDelegate(this, &XTP::API::XtpTrader::OnDisconnected);
				traderevent_managed->OnErrorEvent += gcnew XTP::API::TraderEventSource::OnErrorDelegate(this, &XTP::API::XtpTrader::OnError);
				traderevent_managed->OnQueryAssetEvent += gcnew XTP::API::TraderEventSource::OnQueryAssetDelegate(this, &XTP::API::XtpTrader::OnQueryAsset);
				traderevent_managed->OnTradeEvent += gcnew XTP::API::TraderEventSource::OnTradeDelegate(this, &XTP::API::XtpTrader::OnTrade);
				traderevent_managed->OnOrderEvent += gcnew XTP::API::TraderEventSource::OnOrderDelegate(this, &XTP::API::XtpTrader::OnOrder);
				traderevent_managed->OnOrderCancelEvent += gcnew XTP::API::TraderEventSource::OnOrderCancelDelegate(this, &XTP::API::XtpTrader::OnOrderCancel);
				traderevent_managed->OnQueryOrderEvent += gcnew XTP::API::TraderEventSource::OnQueryOrderDelegate(this, &XTP::API::XtpTrader::OnQueryOrder);
				traderevent_managed->OnQueryTradeEvent += gcnew XTP::API::TraderEventSource::OnQueryTradeDelegate(this, &XTP::API::XtpTrader::OnQueryTrade);
				traderevent_managed->OnQueryPositionEvent += gcnew XTP::API::TraderEventSource::OnQueryPositionDelegate(this, &XTP::API::XtpTrader::OnQueryPosition);
