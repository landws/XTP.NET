...
namespace XTP
{
	namespace API
	{
		...
		///订阅全市场的期权行情应答
			...
			///@param error_info 取消订阅合约时发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
			///@remark 需要快速返回
		public delegate void OnSubscribeAllOptionMarketDataDelegate (EXCHANGE_TYPE exchange_id,RspInfoStruct^ error_info);
		...
		///请求查询期权合约的响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
				...
				///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
		public delegate void  OnQueryOptionAuctionInfoDelegate(QueryOptionAuctionInfoRsp^ option_info, RspInfoStruct^ error_info, int request_id, bool is_last, UInt64 session_id);
	}
}